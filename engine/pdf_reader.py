...
def extract_text_from_pdf(pdf_path, mode="clean"):
    """
    ...
    last_chapter_num = 0
    dynamic_blacklist = []  # 🔥 SAFETY: Initialize here so it exists for all modes

    print(f"The mode selected in pdf_reader.py is {mode}")
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            if mode == "clean":
                ...
                dynamic_blacklist = [txt.lower() for txt, count in noise_frequency.items() if
                                     count > (scan_limit * 0.25)]
            ...
            for page in pdf.pages:
                if mode == "raw":
                    # Grab everything, no margins
                    main_text = page.extract_text(y_tolerance=5)
                    if main_text:  # 🔥 Add this check
                        page_buffers.append(main_text)  # 🔥 Append it to the buffer!
                    continue  # 🔥 Tell the loop to skip to the next page immediately
                else:
                    match = None
